...
```
If you at some point would want to link to a previous page from another one you do this requires some html. Depending on what the first tag you use on that page in the treatment, most likely `<h1>`. That one will need to be given an id. You do that by adding `id=”idname”` inside that tag, though idname is just a placeholder and should be replaced by a more appropriate name.  At this point it should look something like this:
```html
...
```html
<a href="/user/tx/page/3472/#page2>Din text här</a>
```
