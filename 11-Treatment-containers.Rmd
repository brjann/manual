...
```
If you at any point would want to link to a previous page from another one you do this requires some html. Depending on what the first tag you use on that page in the treatment, most likely `<h1>`. That one will need to be given an id. You do that by adding `id=”idname”` inside that tag, though idname is just a placeholder and should be replaced by a more appropriate name.  At this point it should look something like this:
```html
<h1 id=”idname”>your headline here</h1>
```
Now that your page has an id, we can use that to create the link for the first page to get back to this one. You do this by adding /#idname to the end. It should look something like this:
```html
<a href="/user/tx/page/3472/#page2">Your text here</a>
```
Note that it is important to change the link to the correct one for your study. Once that is done, you should be able to move between different pages of your modules, both within the same module and different ones in the same study. 
